...
// WrappedGzipReader checks if r passed-in is a gzip reader wrapped by method DecompressStream
func WrappedGzipReader(r io.Reader) bool {
	if rw, ok := r.(*readCloserWrapper); ok {
		_, isGzip := rw.Reader.(*gzip.Reader)
		return isGzip
	}
	return false
}
